...
/* FIXME(lpereira): Someday I need to go through these warnings and fix them up. */
...
void GifWriteChunk( struct lwan_strbuf* f, GifBitStatus* stat )
{
    lwan_strbuf_append_char(f, (int)stat->chunkIndex);
    lwan_strbuf_append_str(f, stat->chunk, stat->chunkIndex);
...
void GifWriteCode( struct lwan_strbuf* f, GifBitStatus* stat, uint32_t code, uint32_t length )
{
...
void GifWritePalette( const GifPalette* pPal, struct lwan_strbuf* f )
{
    lwan_strbuf_append_char(f, 0);   // first color: transparency
    lwan_strbuf_append_char(f, 0);
    lwan_strbuf_append_char(f, 0);
    ...
    for(int ii=1; ii<(1 << pPal->bitDepth); ++ii)
    {
        ...
        lwan_strbuf_append_char(f, r);
        lwan_strbuf_append_char(f, g);
        lwan_strbuf_append_char(f, b);
    }
}

// write the image header, LZW-compress and write out the image
void GifWriteLzwImage(struct lwan_strbuf* f, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal)
{
    // graphics control extension
    lwan_strbuf_append_char(f, 0x21);
    lwan_strbuf_append_char(f, 0xf9);
    lwan_strbuf_append_char(f, 0x04);
    lwan_strbuf_append_char(f, 0x05); // leave prev frame in place, this frame has transparency
    lwan_strbuf_append_char(f, (delay & 0xff));
    lwan_strbuf_append_char(f, ((delay>>8) & 0xff));
    lwan_strbuf_append_char(f, kGifTransIndex); // transparent color index
    lwan_strbuf_append_char(f, 0);

    lwan_strbuf_append_char(f, 0x2c); // image descriptor block

    lwan_strbuf_append_char(f, left & 0xff);           // corner of image in canvas space
    lwan_strbuf_append_char(f, (left >> 8) & 0xff);
    lwan_strbuf_append_char(f, top & 0xff);
    lwan_strbuf_append_char(f, (top >> 8) & 0xff);

    lwan_strbuf_append_char(f, width & 0xff);          // width and height of image
    lwan_strbuf_append_char(f, (width >> 8) & 0xff);
    lwan_strbuf_append_char(f, height & 0xff);
    lwan_strbuf_append_char(f, (height >> 8) & 0xff);

    //lwan_strbuf_append_char(f, 0); // no local color table, no transparency
    //lwan_strbuf_append_char(f, 0x80); // no local color table, but transparency

    lwan_strbuf_append_char(f, 0x80 + pPal->bitDepth-1); // local color table present, 2 ^ bitDepth entries
    GifWritePalette(pPal, f);

    const int minCodeSize = pPal->bitDepth;
    const uint32_t clearCode = 1 << pPal->bitDepth;

    lwan_strbuf_append_char(f, minCodeSize); // min code size 8 bits
    ...
    lwan_strbuf_append_char(f, 0); // image block terminator
...
bool GifBegin( GifWriter* writer, struct lwan_strbuf *f, uint32_t width, uint32_t height, uint32_t delay, int32_t bitDepth, bool dither )
{
    ...
    lwan_strbuf_append_strz(writer->f, "GIF89a");

    // screen descriptor
    lwan_strbuf_append_char(writer->f, width & 0xff);
    lwan_strbuf_append_char(writer->f, (width >> 8) & 0xff);
    lwan_strbuf_append_char(writer->f, height & 0xff);
    lwan_strbuf_append_char(writer->f, (height >> 8) & 0xff);

    lwan_strbuf_append_char(writer->f, 0xf0);  // there is an unsorted global color table of 2 entries
    lwan_strbuf_append_char(writer->f, 0);     // background color
    lwan_strbuf_append_char(writer->f, 0);     // pixels are square (we need to specify this because it's 1989)

    // now the "global" palette (really just a dummy palette)
    // color 0: black
    lwan_strbuf_append_char(writer->f, 0);
    lwan_strbuf_append_char(writer->f, 0);
    lwan_strbuf_append_char(writer->f, 0);
    // color 1: also black
    lwan_strbuf_append_char(writer->f, 0);
    ...
    {
        // animation header
        lwan_strbuf_append_char(writer->f, 0x21); // extension
        lwan_strbuf_append_char(writer->f, 0xff); // application specific
        lwan_strbuf_append_char(writer->f, 11); // length 11
        lwan_strbuf_append_strz(writer->f, "NETSCAPE2.0"); // yes, really
        lwan_strbuf_append_char(writer->f, 3); // 3 bytes of NETSCAPE2.0 data

        lwan_strbuf_append_char(writer->f, 1); // this is the Netscape 2.0 sub-block ID and it must be 1, otherwise some viewers error
        lwan_strbuf_append_char(writer->f, 0); // loop infinitely (byte 0)
        lwan_strbuf_append_char(writer->f, 0); // loop infinitely (byte 1)

        lwan_strbuf_append_char(writer->f, 0); // block terminator
    }
...
bool GifEnd( GifWriter* writer )
{
    ...
    lwan_strbuf_append_char(writer->f, 0x3b); // end of file
